...
import { IoChevronBack, IoCloseSharp, IoPencilSharp, IoSave } from "react-icons/io5";

export default function TournamentGroup({ params }: { params: { id: string; group: string } }) {
    ...
    function handleUpdateGroup(teamId: number, data: { wins: number; losses: number }) {
        updateGroup({
...
function GroupRow({ team, index, handleUpdateGroup }: any) {
    ...
    return (
        <tr>
            ...
            <td>
                {editing ? (
                    <input type="number" name="wins" onChange={handleChange} value={info?.wins} />
                ) : (
                    team?.wins
                )}
            </td>
