...
export default function useSingleTournament({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    if (playerWonGem) {
      console.log(tournament?.score?.sets.length);
      handleNewGem({ team, setsLength: tournament.score?.sets.length! });

      // TODO: ADD LOGIC FOR GEM COUNTING
      return;
    }
  ...
  function handleNewGem({
    ...
    updateGemScore({
      ...
      gem: setsLength - 1,
      score:
        tournament?.score?.sets[setsLength - 1] === undefined
          ? 0
          : setsLength + 1,
      prevScore: tournament.score?.sets?.[setsLength - 1],
    });
  }
