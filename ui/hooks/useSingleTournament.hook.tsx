...
import { CreateGroupType, GroupType, TeamType, TournamentType } from "@/interfaces/tournaments";
...
export default function useSingleTournament({ id, groupId }: { id: string; groupId?: string }) {
    ...
    // services
    const { mutate: addGroup, isSuccess: isAddGroupSuccess } = useAddGroup(handleError);
    const { mutate: deleteGroup } = useDeleteGroup(handleError);
    const { mutate: updateGroup } = useUpdateGroupPoints(handleError);

    const { mutate: addTeam, isSuccess: isAddTeamSuccess } = useAddTeam(handleError);
    ...
    function handleOnChangeTeam(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        // @ts-ignore
        tmpTeam[player as keyof TeamType][name as keyof PlayerType] = e.target.value;
    ...
    function handleDeleteGroup(groupId: string) {
        deleteGroup({
            tournamentId: id,
            groupId: groupId,
            phase: (queryParams.get("phase") as GroupPhaseEnum) ?? GroupPhaseEnum.GROUPS,
        });
    }
    ...
    return {
        ...
        handleUpdateTeam,
        singleGroup: groupId
            ? (phaseInfo as { [groupId: string]: GroupType })?.[groupId.toUpperCase()]
            : null,
        updateGroup,
        phase,
        matches: groupId
            ? (phaseInfo as { [groupId: string]: GroupType })?.[groupId.toUpperCase()]?.matches
            : null,
        handleDeleteTeam,
        handleDeleteGroup,
    };
