...
import { addGroup, deleteGroup, updateGroup, updateGroupPoints } from "../services/groups";
...
export const useUpdateGroup = (onError: (err: FirebaseError) => void) => {
    return useMutation({
        mutationFn: ({
            tournamentId,
            groupId,
            ...
            phase: string;
            data: { teams: TeamType[] };
        }) => updateGroup({ tournamentId, groupId, phase, data }),
        onError: onError,
    });
};
